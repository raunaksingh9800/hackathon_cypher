...
// 1. RESTORED: This is the correct way to get the route parameter
...
// Lucide Icons
import { XCircle, Loader2, MessageSquare, Bot, User } from 'lucide-react';
...
export default function SimulationDetailPage() {
  ...
  // 2. FIXED: Get route parameters from Next.js correctly
  ...
  // 3. REMOVED: The unnecessary `useState` for simulationId
  // 4. REMOVED: The `useEffect` hack that used `window.location`

  // Fetch data *only* when we have a valid ID
  useEffect(() => {
    ...
    async function fetchSimulation() {
      try {
        setIsLoading(true);
        setError(null);
        // Fetch from the correct API route using a query parameter
        ...
        if (!response.ok) {
          try {
            ...
            // This catches 404s that return HTML, etc.
            throw new Error(
    ...
    fetchSimulation();
  }, [simulationId]); // 5. FIXED: Only depend on `simulationId`. This stops the infinite loop.
